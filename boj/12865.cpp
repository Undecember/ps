/*
 * Author : uuuuuuundec
 * Created at 2024-02-26 18:39:00
...
int _solve() {
    ...
    for (int i = 0; i < N; i++)
        cin >> W[i] >> V[i];
    vector<vector<int>> dpm(N + 1, vector<int>(K + 1, 0));
    for (int i = 0; i < N; i++) {
        for (int k = 0; k <= K; k++) dpm[i + 1][k] = dpm[i][k];
        for (int k = W[i]; k <= K; k++)
            dpm[i + 1][k] = max(dpm[i + 1][k], dpm[i][k - W[i]] + V[i]);
    }
    cout << *max_element(all(dpm[N])) << endl;
    return 0;
}
