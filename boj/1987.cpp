/*
 * Author : uuuuuuundec
 * Created at 2024-02-26 19:06:15
...
int dfs(int i, int j, unsigned int &mask, vector<vector<bool>> &vis) {
    if (vis[i][j]) return 0;
    if (mask & 1U << brd[i][j]) return 0;
    vis[i][j] = true;
    mask |= 1U << brd[i][j];
    ...
    for (int k = 0; k < 4; k++) {
        ...
        res = max(res, dfs(ni, nj, mask, vis));
    }
    mask &= ~(1U << brd[i][j]);
    vis[i][j] = false;
    return res + 1;
...
int _solve() {
    cin >> R >> C;
    for (int i = 0; i < R; i++)
        for (int j = cin.get() * 0; j < C; j++)
            brd[i][j] = cin.get() - 'A' + 1;
    unsigned int mask = 0;
    vector<vector<bool>> vis(R, vector<bool>(C, false));
    cout << dfs(0, 0, mask, vis) << endl;
    return 0;
}
