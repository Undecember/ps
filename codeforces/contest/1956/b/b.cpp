/*
 * Author : undec
 * Created at 2024-04-13 23:46:35
...
int _solve() {
    ...
    sort(all(A));
    set<int> pairs[2], soles[2];
    for (int i = 0; i < N; i++) {
        if (soles[0].find(A[i]) != soles[0].end()) {
            soles[0].erase(A[i]);
            pairs[0].insert(A[i]);
        } else soles[0].insert(A[i]);
    }
    for (int i = 1; i <= N; i++)
        if (soles[0].find(i) != soles[0].end())
            soles[1].insert(i);
        else if (pairs[0].find(i) == pairs[0].end())
            pairs[1].insert(i);
    dbg(pairs[0]);
    dbg(pairs[1]);
    dbg(soles[0]);
    dbg(soles[1]);
    cout << pairs[0].size() << endl;
    return 0;
}
