...
int _solve() {
    ...
    queue<QE> q;
    q.push({0, 0, 0});
    auto cycA = [&](int i, int j) {
        if (j >= M) return 1;
        i = (i % N + N) % N;
        return A[i][j];
    };
    auto cyc = [&](int i, int j, int d) {
        i = (i % N + N) % N;
        j = (j % M + M) % M;
        return QE{i, j, d};
    };
    while (!q.empty()) {
        ...
        int i = qf.i, j = qf.j, d = qf.d;
        if (dist[i][j] <= d) continue;
        dist[i][j] = d;
        if (!cycA(i + 1, j) && !cycA(i + 2, j))
            q.push(cyc(i + 2, j, d + 1));
        if (!cycA(i + 1, j + 1))
            q.push(cyc(i + 1, j + 1, d + 1));
    }
    dbg(dist);
    int ans = INT_MAX;
    for (int i = 0; i < N; i++) {
        ...
        int e = ((d + N - 1) % N);
        int gap = abs(e - i);
        ans = min(ans, dist[i].back() + min(gap, N - gap));
    }
    if (ans >= 1e7) ans = -1;
    cout << ans << endl;
    return 0;
