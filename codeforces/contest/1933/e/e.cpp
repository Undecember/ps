/*
 * Author : uuuuuuundec
 * Created at 2024-02-27 23:35:02
...
int _solve() {
    ...
    cin >> Q;
    lus.resize(Q);
    for (auto &lui : lus) cin >> lui.first >> lui.second;
    vector<int> prfA(N + 1, 0);
    for (int i = 0; i < N; i++) prfA[i + 1] = prfA[i] + A[i];
    auto subsumA = [&](int l, int r) { return prfA[r] - prfA[l]; };
    auto score = [&](int u, int l, int r) {
        int c = subsumA(l, r);
        return (u + u - c + 1) * c / 2;
    };
    for (auto &lui : lus) {
        int L = lui.first - 1, U = lui.second;
        int l = L + 1, r = N + 1;
        while (r - l > 1) {
            int mid = (l + r) / 2;
            if (subsumA(L, mid) < U) l = mid;
            else r = mid;
        }
        int R = l, Rv = score(U, L, R);
        for (int i = l + 1; i < min(l + 3, N + 1); i++) {
            dbg(U);
            dbg(L);
            dbg(i);
            int v = score(U, L, i);
