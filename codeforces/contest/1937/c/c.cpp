...
int _solve() {
    ...
    int mxi = 0;
    for (int i = 1; i < N; i++)
        if (query(mxi, mxi, i, i) == -1)
            mxi = i;
    // find max ors
    vector<int> mxis(1, 0);
    for (int i = 1; i < N; i++) {
        switch(query(mxi, mxis.back(), mxi, i)) {
            case -1:
                mxis.clear();
            case 0:
                mxis.push_back(i);
    ...
    // find min among max ors
    int mnj = 0;
    for (int j = 1; j < (int)mxis.size(); j++)
        if (query(mxis[mnj], mxis[mnj], mxis[j], mxis[j]) == 1)
            mnj = j;
    cout << "! " << mxis[mnj] << ' ' << mxi << endl;
    cout.flush();
    return 0;
}
