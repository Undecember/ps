/*
 * Author : undec
 * Created at 2024-02-27 19:20:01
...
int _solve() {
    ...
    dbg(C);
    vector<vector<int>> aa(2 * N, vector<int>(2 * N, false));
    for (int i = 0; i < 2 * N; i++) {
        for (int j = 0; j < 2 * N; j++) {
            if (C[i][1] == C[j][1])
                aa[i][j] = C[i][0] < C[j][0];
            else if (C[j][1] == T && C[i][1] != T)
                aa[i][j] = true;
        }
    }
    dbg(aa);
    for (int i = 0; i < 2 * N; i++) {
        ...
        function<int(int)> dfs = [&](int ind) {
            ...
            bt.push_back(C[ind]);
            dbg(bt);
            if ((int)bt.size() == 2 * N) return true;
            for (int j = 0; j < 2 * N; j++)
                if (bt.size() % 2 == 0 || aa[ind][j])
