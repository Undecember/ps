...
// To maximize the answer -> split gaps > 2 into gaps == 2 as many as possible
// odd gaps < even gaps
// large even gaps < small even gaps
...
int _solve() {
    ...
    gaps.push_back(N - A.back() + A[0]);
    auto cmp = [&](const int &l, const int &r) {
        if (l % 2 != r % 2) return !(l % 2);
        return l < r;
    };
    sort(all(gaps), cmp);
    vector<int> gapcnt(4);
    for (auto &gap : gaps) {
        int cuts = min((gap - 1) / 2, Y);
        Y -= cuts; X += cuts;
