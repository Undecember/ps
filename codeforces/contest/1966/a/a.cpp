...
int _solve() {
    ...
    {
        map<int, int> mp;
        for (auto &ci : C) mp[ci] = 0;
        for (auto &ci : C) mp[ci]++;
        for (auto &mpi : mp) s.insert({mpi.second, mpi.first});
    }
    while (true) {
        dbg(s);
        auto cur = *s.rbegin();
        ...
        s.erase(prev(s.end()));
        if (cur.first == K && !s.empty()) {
            auto suc = *s.rbegin();
            dbg(suc);
            s.erase(prev(s.end()));
            s.insert({cur.first - 1 + suc.first, suc.second});
            continue;
        }
