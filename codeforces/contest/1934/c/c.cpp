/*
 * Author : undec
 * Created at 2024-03-01 23:35:01
...
int _solve() {
    ...
    int a = query(0, N - 1), b = query(0, 0);
    dbg(a); dbg(b);
    int pivx = max(0, ((a - N + 1) + b + 1) / 2);
    int pivy = b - pivx;
    dbg(pivx); dbg(pivy);
    int c = query(pivx, pivy);
    dbg(c);
    int xq = (pivx + pivy + (a - N + 1) + c) / 2, yq = (pivx + pivy + (a - N + 1) - c) / 2;
    int xr = (pivx - pivy + b - c) / 2, yr = (pivy - pivx + b + c) / 2;
    dbg(xq); dbg(yq);
    dbg(xr); dbg(yr);
    if ((a + b) % 2) {
        if (c % 2) return guess(xq, yq);
        else return guess(xr, yr);
    }
    if (query(xq, yq)) return guess(xr, yr);
    return guess(xq, yq);
}
