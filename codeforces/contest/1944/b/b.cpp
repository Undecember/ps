...
int _solve() {
    ...
    if (c.size() % 2) c.pop_back();
    vector<int> al(all(c));
    dbg(al);
    for (int i = 1; i < N && (int)al.size() < 2 * K; i++)
        if (l[i] == l[i - 1]) {
            al.push_back(l[i]);
            al.push_back(l[i]);
        }
    ...
    vector<int> ar(all(c));
    dbg(ar);
    for (int i = 1; i < N && (int)ar.size() < 2 * K; i++)
        if (r[i] == r[i - 1]) {
            ar.push_back(r[i]);
            ar.push_back(r[i]);
        }
    dbg(ar);
    for (auto &li : al) cout << li << ' ';
    cout << endl;
    for (auto &ri : ar) cout << ri << ' ';
    cout << endl;
    return 0;
}
